...
static void transformHierarTargetList(ParseState *pstate, bool subqryleft,
...
transformHierarStmt(ParseState *pstate, SelectStmt *stmt)
{
	...
	transformHierarTargetList(ps, true, ctequery_l->targetList , NULL);
	...
	/*
	 * transform the target list of cte query. If there were any pseudocolumns,
	 * they have been replaced with appropriate values and an alias has
	 * been created if one was not given. we need to use add these column
	 * alias here to refer to them.
	 */
	pstate->p_cterqry = true;
	foreach(lc, stmt->targetList)
	{
		ResTarget  *res = (ResTarget *) lfirst(lc);
		res->val = resolvePseudoColumns(pstate, (Node *) res->val, res, NULL);
	}
	pstate->p_cterqry = false;


	/*
	 * transform the where clause and qualify the join conditions with
	 * proper relations. if a column in condition is preceded by PRIOR
	 * we qualify it with the cte's name. All other columns are qualified
	 * with actual table name.
	 */
	pstate->p_whereclause = true;
	resolvePseudoColumns(pstate, (Node *) ctequery_r->whereClause, NULL, rv->relname);

	/*
	 * The select statement is now part of cte, and it needs to refer to
	 * the cte in the from clause, instead of the actual table.
	 */
						  ...
						  List *targetList, char *relname)
{
	...
	{
		ResTarget  *res = (ResTarget *) lfirst(lc);
		pstate->p_level = false;
		resolvePseudoColumns(pstate, (Node *) res->val, res, relname);
		if (pstate->p_level)
